...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $flight_id = validate_input($_POST["flight_id"]);
    ...
    if (empty($flight_number_err) && empty($airline_err) && empty($origin_err) && empty($destination_err) && empty($departure_time_err) && empty($arrival_time_err)) {
        $sql = "UPDATE flights SET flight_number=?, airline=?, origin=?, destination=?, departure_time=?, arrival_time=? WHERE id=?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssssi", $flight_number, $airline, $origin, $destination, $departure_time, $arrival_time, $flight_id);
